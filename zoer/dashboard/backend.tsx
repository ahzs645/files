...
import { queryCatalog, readAll } from './queries';
...
export function refresh() {
  ...
  pending = (async () => {
    try {
      ...
      snapshot = { model };
    } catch (error) { snapshot = { ...snapshot, error: error instanceof Error ? error.message : 'Unable to read Zoer data.' }; }
    finally { pending = undefined; emit(); }
  })();
  return pending;
}
...
export function setStar(entity: 'opportunity' | 'award', key: string, starred: boolean) {
  ...
  })().finally(() => { starPending.delete(id); snapshot = { ...snapshot }; emit(); });
