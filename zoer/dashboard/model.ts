...
export function buildModel(state: WorkspaceState, documents: SavedDocument[]) {
  ...
  const runs = state.runs.filter(run => ['scrape.full', 'scrape.sample', 'listing.capture', 'detail.capture'].includes(run.actionId)).map(run => {
    ...
    const message = run.error || (status === 'succeeded' ? `${counts.listingCount} listings · ${counts.detailCount} details · ${full ? 'all current public opportunities' : 'bounded capture'}` : status === 'cancelled' ? 'Stopped by operator' : doc ? `Saved ${counts.listingCount} listings across ${doc.currentPage ?? 1} pages · ${counts.detailCount} details${full ? ` · ${doc.pending?.length ?? 0} remaining` : ""}` : 'Opening BC Bid and collecting the listing page');
    return { _id: run.id, status, trigger: 'manual', startedAt: Date.parse(run.createdAt), completedAt: run.completedAt ? Date.parse(run.completedAt) : null,
      cancellationRequested: !!run.cancelRequestedAt, counts, errorMessage: run.error || doc?.error, errorCode: status === 'failed' ? 'scrape_failed' : null,
  ...
  return { stars, awardCheckpoint, awardRuns, opportunities: [...opportunities.values()].map(row => ({ ...row, starred: stars.get('opportunity:' + row.sourceKey) === true })), awards: [...awards.values()].map(row => ({ ...row, starred: stars.get('award:' + row.importKey) === true })), runs, history, runsTruncated: state.runsTruncated };
}
...
export function queryModel(model: Model, name: string, args: any = {}): any {
  ...
  if (name === 'dashboard.summary') return { total: opportunities.length, open: opportunities.filter(row => /open/i.test(row.status)).length,
    ...
    organizations: new Set(opportunities.map(row => row.issuedBy).filter(Boolean)).size,
  ...
  if (name === 'opportunities.getByProcessId') return opportunities.find(row => (row.processId ?? row.sourceKey) === args.processId) ?? null;
  if (name === 'opportunities.listByRunId') return [...(model.history.get(args.runId)?.values() ?? [])].slice(0, args.limit ?? 50).map(row => ({ ...row, detailFieldCount: row.detailFields.length, addendaCount: row.addenda.length, attachmentCount: row.attachments.length }));
  if (name === 'opportunities.list') {
    const rows = opportunities.filter(row => (!args.starredOnly || row.starred) && (!args.status || row.status === args.status) && (!args.type || row.type === args.type) && (!args.issuedBy || row.issuedBy === args.issuedBy)
      ...
      && (!args.search || [row.description, row.opportunityId, row.issuedBy, row.searchText].join(' ').toLowerCase().includes(args.search.toLowerCase())))
    ...
    return { items: rows.slice(offset, offset + limit), nextCursor: offset + limit < rows.length ? String(offset + limit) : null, total: rows.length };
  }
  if (name === 'contractAwards.summary') { const latest = [...awards].sort((a, b) => b.updatedAt - a.updatedAt)[0]; return { total: awards.length, organizations: new Set(awards.map(row => row.issuingOrganization).filter(Boolean)).size, suppliers: new Set(awards.map(row => row.successfulSupplier).filter(Boolean)).size, latestImportAt: latest?.updatedAt ?? null, latestImportFile: latest?.sourceFileName ?? null }; }
  if (name === 'contractAwards.list') { const rows = awards.filter(row => !args.search || row.searchText.includes(args.search.toLowerCase())).sort((a,b) => b.updatedAt - a.updatedAt); const limit = args.limit ?? 200; return { items: rows.slice(0, limit), nextCursor: rows.length > limit ? 'truncated' : null, hasMore: rows.length > limit, total: rows.length }; }
  if (name === 'contractAwardsAnalysis.overview') return buildContractAwardAnalysisOverview(awards, args);
  if (name === 'contractAwardsAnalysis.supplierProfile') return buildContractAwardEntityProfile(awards, 'supplier', args.supplierKey, args.filters);
  if (name === 'contractAwardsAnalysis.organizationProfile') return buildContractAwardEntityProfile(awards, 'organization', args.organizationKey, args.filters);
  if (name === 'contractAwardsAnalysis.entityOptions') return buildContractAwardEntityOptions(awards, args.kind, args.search, args.includePlaceholderSuppliers ?? false);
