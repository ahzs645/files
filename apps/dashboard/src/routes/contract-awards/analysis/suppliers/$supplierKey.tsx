...
export const Route = createFileRoute(
  ...
  component: SupplierAnalysisDetailPage,
});

function SupplierAnalysisDetailPage() {
  const { supplierKey } = Route.useParams();
  const [filters, setFilters] = useState<ContractAwardAnalysisFilters>(
    DEFAULT_ANALYSIS_FILTERS,
  );
  const runSupplierProfile = useAction(api.contractAwardsAnalysis.supplierProfile);
  ...
  useEffect(() => {
    ...
    void runSupplierProfile({
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Could not load supplier analysis.");
        }
      });
  ...
  }, [filters, runSupplierProfile, supplierKey]);
  ...
  if (profile === undefined) {
    return (
      <div className="rounded-2xl border border-dashed border-border-default px-6 py-16 text-center text-sm text-text-tertiary">
        {error ?? "Could not load supplier analysis."}
      </div>
    );
  }
  ...
  if (profile === null) {
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-dashed border-border-default px-6 py-16 text-center text-sm text-text-tertiary">
          Supplier not found in the imported contract awards dataset.
        </div>
      </div>
    );
  }
