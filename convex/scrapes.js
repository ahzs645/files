...
async function postInternal(path, body) {
    ...
    if (!baseUrl || !token) {
        throw new Error("Missing SCRAPER_INTERNAL_BASE_URL or SCRAPER_INTERNAL_TOKEN in Convex environment variables.");
    }
...
export const triggerNow = action({
    args: {},
    handler: async () => {
        return await postInternal("/internal/scrape", { trigger: "manual" });
    }
});
export const stopActive = action({
    args: {},
    handler: async () => {
        return await postInternal("/internal/stop", {});
    }
});
