...
async function findExistingOpportunity(ctx, opportunity) {
    if (opportunity.processId) {
        const byProcessId = await ctx.db
            .query("opportunities")
            .withIndex("by_processId", (queryBuilder) => queryBuilder.eq("processId", opportunity.processId))
            .unique();
        if (byProcessId) {
            return byProcessId;
        }
    ...
    return await ctx.db
        .query("opportunities")
        .withIndex("by_opportunityId", (queryBuilder) => queryBuilder.eq("opportunityId", opportunity.opportunityId))
        .unique();
}
async function replaceChildren(ctx, table, sourceKey, rows) {
    const existing = await ctx.db
        .query(table)
        .withIndex("by_sourceKey", (queryBuilder) => queryBuilder.eq("sourceKey", sourceKey))
        .collect();
...
export const startRun = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const latestActive = [...running, ...stopping].sort((left, right) => right.startedAt - left.startedAt)[0] ?? null;
        if (latestActive) {
...
export const upsertOpportunityBatch = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const item of args.batch) {
            ...
            await replaceChildren(ctx, "addenda", childSourceKey, item.addenda.map((addendum) => ({
                sourceKey,
                processId: item.processId,
            ...
            await replaceChildren(ctx, "attachments", childSourceKey, item.attachments.map((attachment) => ({
                sourceKey,
                processId: item.processId,
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.patch(args.runId, {
            ...
            progress: {
                ...
                current: args.status === "succeeded" || args.status === "cancelled"
