import {
  buildContractAwardEntityKey,
  isPlaceholderContractAwardSupplier,
  normalizeContractAwardEntityLabel,
  parseContractAwardDate,
  type ContractAwardAnalysisEntityKind,
  type ContractAwardAnalysisFilters,
  type ContractAwardAppliedAnalysisFilters,
  type ContractAwardAnalysisOverview,
  type ContractAwardBenchmark,
  type ContractAwardBreakdownRow,
  type ContractAwardDataQualitySummary,
  type ContractAwardEntityOption,
  type ContractAwardEntityProfile,
  type ContractAwardFinding,
  type ContractAwardFindingSeverity,
  type ContractAwardListItem,
  type ContractAwardRankingRow,
  type ContractAwardSummaryMetrics,
  type ContractAwardTrendPoint,
...
function shiftYears(dateString: string, years: number) {
  const date = new Date(`${dateString}T00:00:00.000Z`);
  date.setUTCFullYear(date.getUTCFullYear() + years);
  return toDateString(date);
}

function normalizeAwardLabel(value: string | null | undefined, fallback: string) {
...
function createSummaryMetrics(items: AwardDoc[]): ContractAwardSummaryMetrics {
  const values = items
  ...
  return {
    ...
    uniqueSuppliers: new Set(
      items.map((item) => normalizeAwardLabel(item.successfulSupplier, "Unknown supplier")),
    ).size,
    uniqueIssuingOrganizations: new Set(
      items.map((item) => normalizeAwardLabel(item.issuingOrganization, "Unknown organization")),
    ).size,
    averageAwardValue: totalAwards > 0 ? totalAwardValue / totalAwards : null,
    medianAwardValue: median(values),
    contractNumberCoverage: roundRate(
      items.filter((item) => Boolean(item.contractNumber)).length,
      totalAwards,
    ),
    justificationCoverage: roundRate(
      items.filter((item) => Boolean(item.justification)).length,
      totalAwards,
    ),
    placeholderSupplierShare: roundRate(
      items.filter((item) => isPlaceholderContractAwardSupplier(item.successfulSupplier)).length,
      totalAwards,
    ),
  };
}

function createDataQualitySummary(items: AwardDoc[]): ContractAwardDataQualitySummary {
  const today = getTodayDateString();
  const totalRows = items.length;
  const placeholderSupplierCount = items.filter((item) =>
  ...
  const futureDatedAwardCount = items.filter(
    (item) => item.awardDate && item.awardDate > today,
  ).length;
...
function createRankingRow(
  ...
  placeholder: boolean,
): ContractAwardRankingRow {
  const breakdown = createBreakdownRow(key, label, items, totalAwards, totalValue);

  return {
    ...breakdown,
    averageValue:
      breakdown.awardCount > 0 ? breakdown.totalValue / breakdown.awardCount : null,
    secondaryLabel,
    placeholder,
  };
}
...
): ContractAwardAppliedAnalysisFilters {
...
function getTrendBucket(filters: ContractAwardAppliedAnalysisFilters): TrendBucket {
  ...
  if (filters.datePreset === "custom" && filters.fromDate && filters.toDate) {
    const diffDays = Math.max(
      0,
      Math.round(
        (new Date(`${filters.toDate}T00:00:00.000Z`).getTime() -
...
export function applyContractAwardAnalysisFilters(
  docs: AwardDoc[],
  filters: ContractAwardAnalysisFilters,
): FilterResult {
  ...
  const items = docs.filter((item) => {
    if (
      !appliedFilters.includePlaceholderSuppliers &&
      isPlaceholderContractAwardSupplier(item.successfulSupplier)
    ) {
      return false;
    }
    ...
    if (
      appliedFilters.opportunityType &&
    ...
    if (
      appliedFilters.minimumAwardValue !== null &&
...
function getPeriodStart(value: string, bucket: TrendBucket) {
...
function getPeriodEnd(start: string, bucket: TrendBucket) {
...
function getPeriodLabel(start: string, bucket: TrendBucket) {
  const date = new Date(`${start}T00:00:00.000Z`);
  if (bucket === "month") {
...
function createTrendPoints(items: AwardDoc[], bucket: TrendBucket): ContractAwardTrendPoint[] {
  const grouped = new Map<string, AwardDoc[]>();
  ...
  return [...grouped.entries()]
    ...
    .map(([periodStart, periodItems]) => ({
      ...
      totalValue: periodItems.reduce(
        (sum, item) => sum + (item.contractValue ?? 0),
        0,
      ),
    }));
}

function createRankingRowsForKind(
  items: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
  sortBy: "value" | "count",
  ...
  const rows = [...groups.entries()].map(([label, groupItems]) =>
    createRankingRow(
      buildContractAwardEntityKey(label),
      label,
      groupItems,
      totalAwards,
      totalValue,
      null,
      kind === "supplier" && isPlaceholderContractAwardSupplier(label),
    ),
  );
...
function createConcentrationRows(items: AwardDoc[]): ContractAwardBreakdownRow[] {
  const totalValue = items.reduce((sum, item) => sum + (item.contractValue ?? 0), 0);
...
function createSupplierDiversityDistribution(
  items: AwardDoc[],
): ContractAwardBreakdownRow[] {
  const supplierGroups = groupBy(items, (item) => getFieldForKind(item, "supplier"));
  const bins = new Map<string, AwardDoc[]>([
    ["single", []],
    ["few", []],
    ["multi", []],
    ["broad", []],
  ...
  for (const [, groupItems] of supplierGroups) {
    const organizationCount = new Set(
      groupItems.map((item) => getFieldForKind(item, "organization")),
    ).size;
    const bucketKey =
      organizationCount <= 1
        ? "single"
  ...
  return [...bins.entries()].map(([key, bucketItems]) => ({
    ...
    shareOfAwards: roundRate(
      bucketItems.length > 0
        ? new Set(bucketItems.map((item) => getFieldForKind(item, "supplier"))).size
        : 0,
      totalSuppliers,
    ),
    shareOfValue: roundRate(
      bucketItems.reduce((sum, item) => sum + (item.contractValue ?? 0), 0),
      totalValue,
    ),
  }));
}

function createTopSupplierConcentrationRows(
  items: AwardDoc[],
): ContractAwardRankingRow[] {
  ...
  const rows = [...organizationGroups.entries()].map(([label, groupItems]) => {
    ...
    return {
      ...
      placeholder: false,
    } satisfies ContractAwardRankingRow;
  });
...
function createOrganizationTypeMixRows(items: AwardDoc[]): ContractAwardTypeMixRow[] {
  const topOrganizations = createRankingRowsForKind(
    items,
    "organization",
    "value",
    ORGANIZATION_TYPE_MIX_LIMIT,
  );
  const organizationGroups = groupBy(items, (item) => getFieldForKind(item, "organization"));
  ...
  return topOrganizations.map((row) => {
    ...
    const typeGroups = groupBy(
      groupItems,
      (item) => normalizeAwardLabel(item.opportunityType, "Unknown type"),
    );
    const totalAwards = groupItems.length;
    const totalValue = groupItems.reduce((sum, item) => sum + (item.contractValue ?? 0), 0);
...
function createOverviewFindings(
  summary: ContractAwardSummaryMetrics,
  dataQuality: ContractAwardDataQualitySummary,
  supplierConcentration: ContractAwardBreakdownRow[],
): ContractAwardFinding[] {
  const findings: ContractAwardFinding[] = [];
  const top10Share =
    supplierConcentration.find((row) => row.key === "top10")?.shareOfValue ?? 0;
  ...
  if (summary.placeholderSupplierShare >= 0.25) {
    findings.push({
      ...
      description:
        "A large share of awards use placeholder supplier names, so supplier league tables should be interpreted with care.",
      severity: "warning",
    });
  }
  ...
  if (dataQuality.missingContractNumberRate >= 0.4) {
    findings.push({
      ...
      description:
        "Many awards do not include a contract number, which weakens downstream matching and auditability.",
      severity: "warning",
    });
  }
  ...
  if (dataQuality.missingJustificationRate >= 0.8) {
    findings.push({
      ...
      description:
        "Justification is best treated as a descriptive coverage field, not a reliable comparative signal.",
      severity: "info",
    });
  }
  ...
  if (dataQuality.futureDatedAwardCount > 0) {
    findings.push({
      ...
      description:
        "Some award dates fall after today and should be reviewed as potential source anomalies or pre-award records.",
      severity: "warning",
    });
  }
  ...
  if (top10Share >= 0.6) {
    findings.push({
      ...
      description:
        "A majority of award value is concentrated in the top 10 suppliers after current filters are applied.",
      severity: "info",
    });
  }
...
function sortAwardsForTable(items: AwardDoc[]): ContractAwardListItem[] {
  return [...items]
    .sort((left, right) => {
...
function createOpportunityTypeBreakdown(items: AwardDoc[]): ContractAwardBreakdownRow[] {
  const totalAwards = items.length;
  const totalValue = items.reduce((sum, item) => sum + (item.contractValue ?? 0), 0);
...
function createAwardSizeDistribution(items: AwardDoc[]): ContractAwardBreakdownRow[] {
  const bins = [
...
function calculateConcentrationToTopCounterparty(
  items: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
) {
...
function createCounterpartyRankings(
  items: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
): ContractAwardRankingRow[] {
  const totalAwards = items.length;
  ...
  return [...groups.entries()]
    .map(([label, groupItems]) =>
      createRankingRow(
        ...
        totalValue,
        null,
        kind === "organization" && isPlaceholderContractAwardSupplier(label),
      ),
    )
    .sort((left, right) => right.totalValue - left.totalValue)
    .slice(0, TOP_LIST_LIMIT);
...
function createBenchmark(
  filteredItems: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
  entityKey: string,
): ContractAwardBenchmark {
  const entityGroups = groupBy(filteredItems, (item) => getFieldForKind(item, kind));
  const targetLabel = [...entityGroups.keys()].find(
    (label) => buildContractAwardEntityKey(label) === entityKey,
  );
...
function createEntityFindings(
  ...
  if (kind === "supplier") {
    if ((concentration ?? 0) >= 0.65) {
      pushFinding(
        findings,
        "high-issuer-dependence",
        "High dependence on one issuing organization",
        "This supplier draws a large share of award value from a single issuing organization under the current filters.",
        "warning",
      );
    }
    ...
    if (dataQuality.missingContractNumberRate >= 0.5) {
      pushFinding(
        findings,
        "supplier-contract-number-coverage",
        "Contract number coverage is weak",
        "More than half of the supplier's filtered awards are missing a contract number.",
        "warning",
      );
    }
    ...
    if (dataQuality.futureDatedAwardCount > 0) {
      pushFinding(
        findings,
        "supplier-future-awards",
        "Future-dated awards are present",
        "At least one award tied to this supplier is dated in the future.",
        "warning",
      );
    }
    ...
    if (uniqueCounterparties <= 2 && items.length > 0) {
      pushFinding(
        findings,
        "supplier-low-diversity",
        "Issuer diversity is low",
        "This supplier appears across very few issuing organizations in the current slice.",
        "info",
      );
    }
  } else {
    if ((concentration ?? 0) >= 0.5) {
      pushFinding(
        findings,
        "org-supplier-concentration",
        "One supplier dominates award value",
        "A single supplier accounts for a large share of filtered award value for this issuing organization.",
        "warning",
      );
    }
    ...
    if (dataQuality.placeholderSupplierRate >= 0.3) {
      pushFinding(
        findings,
        "org-placeholder-suppliers",
        "Placeholder suppliers materially affect this issuer",
        "Placeholder supplier names represent a meaningful share of this issuer's filtered awards.",
        "warning",
      );
    }
    ...
    if (dataQuality.missingContractNumberRate >= 0.5) {
      pushFinding(
        findings,
        "org-contract-number-coverage",
        "Contract number coverage is weak",
        "More than half of the filtered awards for this issuing organization are missing a contract number.",
        "warning",
      );
    }
    ...
    if (dataQuality.futureDatedAwardCount > 0) {
      pushFinding(
        ...
        "org-future-awards",
        "Future-dated awards are present",
        "At least one filtered award for this issuing organization is dated in the future.",
        "warning",
      );
    }
  }
  ...
  if (
    benchmark.peerMedianConcentration !== null &&
    concentration !== null &&
    concentration > benchmark.peerMedianConcentration * 1.35
  ) {
    pushFinding(
      findings,
      "peer-concentration-outlier",
      "Counterparty concentration is above the peer median",
      "Relative to peers under the same filters, this entity is more concentrated in a single counterparty.",
      "info",
    );
  }
...
function getTypeOptions(items: AwardDoc[]): string[] {
  return [
    ...new Set(
      items
...
export function buildContractAwardAnalysisOverview(
  docs: AwardDoc[],
  filters: ContractAwardAnalysisFilters,
): ContractAwardAnalysisOverview {
  const typeOptions = getTypeOptions(docs);
  const { appliedFilters, trendBucket, items } = applyContractAwardAnalysisFilters(
    docs,
    filters,
  );
...
export function buildContractAwardEntityProfile(
  docs: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
  entityKey: string,
  filters: ContractAwardAnalysisFilters,
): ContractAwardEntityProfile | null {
  const typeOptions = getTypeOptions(docs);
  const { appliedFilters, trendBucket, items } = applyContractAwardAnalysisFilters(
    docs,
    filters,
  );
  const targetLabel: string | undefined = docs
  ...
  const entityItems = items.filter(
    (item) => buildContractAwardEntityKey(getFieldForKind(item, kind)) === entityKey,
  );
  ...
  return {
    ...
    summary: {
      ...
      uniqueCounterparties: new Set(
        entityItems.map((item) => getCounterpartyLabel(item, kind)),
      ).size,
      uniqueOpportunityTypes: new Set(
        entityItems.map((item) => normalizeAwardLabel(item.opportunityType, "Unknown type")),
      ).size,
    ...
    benchmark,
    findings: createEntityFindings(
      kind,
      entityItems,
      dataQuality,
      concentration.share,
      new Set(entityItems.map((item) => getCounterpartyLabel(item, kind))).size,
      benchmark,
    ),
    dataQuality,
...
export function buildContractAwardEntityOptions(
  docs: AwardDoc[],
  kind: ContractAwardAnalysisEntityKind,
  ...
  return [...groups.entries()]
    ...
    .filter((item) =>
      normalizedSearch ? item.label.toLowerCase().includes(normalizedSearch) : true,
    )
    .sort((left, right) => {
      if (left.totalValue !== right.totalValue) {
        return right.totalValue - left.totalValue;
