...
export function sortOpportunities(left, right) {
...
export function sortContractAwards(left, right) {
    ...
    return (left.opportunityId ?? left.opportunityDescription).localeCompare(right.opportunityId ?? right.opportunityDescription);
}
export function isClosingSoon(closingDate) {
    if (!closingDate) {
        return false;
    }
...
export function normalizeScrapeRun(run) {
    if (!run) {
        return null;
    }
    return {
