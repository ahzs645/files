...
export const active = query({
  ...
  handler: async (ctx) => {
    ...
    return normalizeScrapeRun(
      [running, stopping].filter(Boolean).sort((left, right) => right!.startedAt - left!.startedAt)[0] ?? null
    );
  }
});
...
export const listRecent = query({
  ...
  handler: async (ctx, args) => {
    ...
    return (await ctx.db.query("scrapeRuns").withIndex("by_startedAt").order("desc").take(limit)).map((run) =>
      normalizeScrapeRun(run)
    );
  }
});
