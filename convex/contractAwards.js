import { buildContractAwardImportKey, buildContractAwardSearchText, hasMeaningfulContractAwardData, normalizeContractAwardImportRecord, parseContractAwardValue, } from "@bcbid/shared";
...
function mapContractAwardListItem(doc) {
    return {
...
export const list = action({
    ...
    handler: async (ctx, args) => {
        ...
        if (!search) {
            const page = await ctx.runQuery(internal.contractAwardsAnalysisPaging.loadBrowsePage, {
                paginationOpts: {
                    cursor: null,
                    numItems: limit + 1,
...
export const summary = action({
    ...
    handler: async (ctx) => {
        ...
        const latestImport = await ctx.runQuery(internal.contractAwardsAnalysisPaging.getLatestImport, {});
        return {
            total,
    ...
    handler: async (ctx, args) => {
        ...
        for (const [importKey, record] of dedupedRecords) {
            ...
            const existing = await ctx.db
                ...
                .withIndex("by_importKey", (queryBuilder) => queryBuilder.eq("importKey", importKey))
                .unique();
            if (existing) {
