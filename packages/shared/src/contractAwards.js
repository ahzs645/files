...
function rowsPayloadToObjects(headers, rows) {
    ...
    return rows
        ...
        .map((row) => Object.fromEntries(safeHeaders.map((header, index) => [header, row[index] ?? null])));
}
...
export function normalizeContractAwardImportRecord(record) {
    return {
...
export function hasMeaningfulContractAwardData(record) {
    return isMeaningfulRecord(normalizeContractAwardImportRecord(record));
}
export function parseContractAwardValue(value) {
...
export function buildContractAwardImportKey(record) {
    ...
    const valueComponent = normalized.contractNumber
        ...
        : String(parseContractAwardValue(normalized.contractValueText) ??
...
export function buildContractAwardSearchText(record) {
    const normalized = normalizeContractAwardImportRecord(record);
    return [
        normalized.opportunityId,
        normalized.opportunityDescription,
...
export function parseContractAwardsJson(jsonText) {
    const payload = JSON.parse(jsonText);
    const rows = extractRows(payload);
    return rows
        .map((row) => normalizeContractAwardImportRecord({
        opportunityId: readOptionalCell(row, "opportunityId"),
        opportunityDescription: readOptionalCell(row, "opportunityDescription") ?? "",
