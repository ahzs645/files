...
import type { ListingPageParseResult, OpportunityListing } from "../types";
import {
  dedupeStrings,
  extractProcessId,
  normalizeWhitespace,
...
function parsePagerValue($: cheerio.CheerioAPI): { currentPage: number; totalPages: number } {
  const hiddenValue = Number.parseInt(
    normalizeWhitespace(
  ...
  const hiddenMaxPageIndex = Number.parseInt(
    normalizeWhitespace(
      $("input[name='maxpageindexbody_x_grid_grd'], #maxpageindexbody_x_grid_grd")
        .first()
        .attr("value")
    ),
    10
...
export function parseListingPage(html: string, baseUrl: string): ListingPageParseResult {
